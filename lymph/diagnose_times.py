...
class Distribution:
    ...
    @property
    def distribution(self, n_mixture_components: int = 2) -> np.ndarray:
        """Return the probability mass function of the distribution if it is frozen."""
        if self._kwargs == {} or not isinstance(list(self._kwargs.values())[0], list):
            ...
            if not hasattr(self, "_frozen") or self._frozen is None:
                _dists = []
                # somehow messy to extract the number of cluster:
                n_clusters = len(list(self._kwargs.values())[0])
                for i in range(n_clusters):
                    params = {k: v[i] for k, v in self._kwargs.items()}
                    _dists.append(self.normalize(self._func(self.support, **params)))
                dists = np.stack(_dists, axis=0)
                self._frozen = dists
            else:
                if not self._frozen.ndim > 1:
                    self._frozen = expand_to_dimension(self._frozen, len(list(self._kwargs.values)[0]))
            return self._frozen
