...
from lysubsite.core.model_functions import compute_K_matrices, compute_PI_matrices, compute_SP_matrices, create_D_matrix
...
class LymphMixtureModel:
    ...
    def _mm_hmm_likelihood(self, DS, KS, log: bool = True) -> float:
        """
        Implements the likelihood function
        """
        llh = 0
        # Sum over all subsites..
        for DT, KT in zip(DS, KS):
            # .. and sum over all t_stages within that subsite.
            for D, K in zip(DT, KT):
                # sum the likelihoods of observing each patient diagnose given the mixture parameters.
                llh += np.sum(np.log(K @ D))
        return llh

    def mm_hmm_likelihood(self, thetas, cluster_assignments = None, data_models = None, n_clusters = None, DS = None):
        """ Calculates the likelihood of observing the data using mixture model. """
        # t_stages = MODELS[0].t_stages
        # Generate the Diagnose Matrices
        if DS is None:
            DS = create_D_matrix(data_models)
        
        # compute the SP Matrices 
        t_stages = list(data_models[0].t_stages)
        SP = compute_SP_matrices(thetas, MODELS, t_stages ) 

        # restructure cluster_assignments
        PI = compute_PI_matrices(cluster_assignments, n_S=len(data_models), n_K=len(MODELS))
        # Generate the Kappa Matrices
        KS = compute_K_matrices(SP, PI)
        return self._mm_hmm_likelihood(DS, KS)
